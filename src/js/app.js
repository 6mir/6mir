...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // load picture
  const srcerror = "./src/picture/error/img.png";

  const imgallitem = document.querySelectorAll(".imglod");
  const lodallitem = document.querySelectorAll(".loaderlod");

  imgallitem.forEach((imgitem, loaderitemall) => {
    const loditem = lodallitem[loaderitemall];

    imgitem.src = imgitem.src || srcerror;

    imgitem.onload = function () {
      imgitem.classList.remove("hidden");
      loditem.classList.add("hidden");
    };

    imgitem.onerror = function () {
      imgitem.src = srcerror;
    };
  });
});
...
function copyToClipboard(text) {
  navigator.clipboard
    ...
    .then(() => {
      ...
      }, 1000); // پیام بعد از 2 ثانیه محو می‌شود
    })
    .catch((err) => {
